...
class Themeton_Nav_Menu_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'description' => esc_html__('Add a custom menu to your sidebar.', 'press-grid'),
			'customize_selective_refresh' => true,
		);
		parent::__construct( false, esc_html__(': Menu for Push Sidebar', 'press-grid'), $widget_ops );
	}
	...
	public function form( $instance ) {
		...
		<p class="nav-menu-widget-no-menus-message" <?php if ( ! empty( $menus ) ) { print ' style="display:none" '; } ?>>
			...
			?>
			<?php printf('%s <a href="%s">%s</a>', esc_html__('No menus have been created yet.', 'press-grid'), esc_attr($url), esc_html__('Create some', 'press-grid')); ?>
		</p>
		<div class="nav-menu-widget-form-controls" <?php if ( empty( $menus ) ) { print ' style="display:none" '; } ?>>
			<p>
				<label for="<?php print $this->get_field_id( 'title' ); ?>"><?php esc_html_e('Title:', 'press-grid') ?></label>
			...
			<p>
				<label for="<?php print $this->get_field_id( 'nav_menu' ); ?>"><?php esc_html_e('Select Menu:', 'press-grid'); ?></label>
				<select id="<?php print $this->get_field_id( 'nav_menu' ); ?>" name="<?php print $this->get_field_name( 'nav_menu' ); ?>">
					<option value="0"><?php esc_html_e('&mdash; Select &mdash;', 'press-grid'); ?></option>
			...
			<p>
				<label for="<?php print $this->get_field_id( 'menu_style' ); ?>"><?php esc_html_e('Menu output style:', 'press-grid'); ?></label>
				<select id="<?php print $this->get_field_id( 'menu_style' ); ?>" name="<?php print $this->get_field_name( 'menu_style' ); ?>">
					<option value="small" <?php print esc_attr($menu_style=='small' ? 'selected' : ''); ?>><?php esc_html_e('Small Menu', 'press-grid'); ?></option>
					<option value="medium" <?php print esc_attr($menu_style=='medium' ? 'selected' : ''); ?>><?php esc_html_e('Medium Menu + Icon', 'press-grid'); ?></option>
					<option value="big" <?php print esc_attr($menu_style=='big' ? 'selected' : ''); ?>><?php esc_html_e('Big Menu', 'press-grid'); ?></option>
				</select>
			</p>
			<?php if ( $wp_customize instanceof WP_Customize_Manager ) : ?>
				<p class="edit-selected-nav-menu" style="<?php if ( ! $nav_menu ) { print 'display: none;'; } ?>">
					<button type="button" class="button"><?php esc_html_e('Edit Menu', 'press-grid') ?></button>
				</p>
			<?php endif; ?>
		</div>
		<?php
